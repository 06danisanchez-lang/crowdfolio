...
export function useAssets() {
  ...
  const fetchAssets = useCallback(async () => {
    ...
    const timeoutId = setTimeout(() => {
      ...
      setIsLoading(false);
      setError('Timeout: la carga de activos tardó demasiado');
    }, FETCH_TIMEOUT_MS);
  ...
  const addAsset = async (asset: Omit<Asset, 'id' | 'userId' | 'createdAt' | 'updatedAt'>) => {
    ...
    try {
      const { data, error } = await supabase.from('assets').insert({
        user_id: user.id, platform_name: asset.platformName, project_name: asset.projectName,
        country_code: asset.countryCode, asset_type: asset.assetType as AssetType,
        acquisition_cost: asset.acquisitionCost, investment_date: asset.investmentDate,
        expected_end_date: asset.expectedEndDate || null, expected_return: asset.expectedReturn,
        status: asset.status, notes: asset.notes || null,
      }).select().single();
      if (error) throw error;
      const newAsset = assetFromRow(data as AssetRow);
      setAssets((prev) => [newAsset, ...prev]);
      toast({ title: 'Activo añadido', description: `${asset.projectName} ha sido registrado correctamente` });
      return newAsset;
    } catch (error) {
      console.error('Error adding asset:', error);
      toast({ title: 'Error', description: 'No se pudo añadir el activo', variant: 'destructive' });
      return null;
    }
  };
  ...
  const updateAsset = async (id: string, updates: Partial<Omit<Asset, 'id' | 'userId' | 'createdAt' | 'updatedAt'>>) => {
    try {
      ...
      toast({ title: 'Activo actualizado', description: 'Los cambios se han guardado correctamente' });
      return updatedAsset;
    } catch (error) {
      console.error('Error updating asset:', error);
      toast({ title: 'Error', description: 'No se pudo actualizar el activo', variant: 'destructive' });
      return null;
    }
  };
  ...
  const deleteAsset = async (id: string) => {
    try {
      ...
      toast({ title: 'Activo eliminado', description: 'El activo y sus transacciones han sido eliminados' });
      return true;
    } catch (error) {
  ...
  return {
    ...
    refetch: fetchAssets,
    totalInvested, lendingAssets, equityAssets, foreignAssets, foreignAssetsValue,
  };
}
