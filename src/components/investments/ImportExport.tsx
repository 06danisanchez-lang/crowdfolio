  ...
  const handleImportJSON = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        setImportOpen(false);
        setImportError(null);
        toast.success(`${data.length} inversiones importadas`);
      } catch (error) {
  ...
  const handleImportCSV = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const investments: Investment[] = lines.slice(1).map((line, index) => {
        ...
        onImport(investments, false);
        setImportOpen(false);
        setImportError(null);
        toast.success(`${investments.length} inversiones importadas desde CSV`);
      } catch (error) {
